...
For cluster reruns where FAISS is intentionally skipped, the core benchmark CSV contracts below still hold. In that case:

- `results/cluster/<run-tag>/faiss/` may be absent or intentionally empty
- `results/cluster/<run-tag>/analysis/faiss_analysis.csv` may contain only the header row
- the paired Markdown review under `analysis/final_conclusions.md` or `docs/analysis/latest-cluster-benchmark-review.md` explicitly states that FAISS comparison was skipped for that run
...
The same schemas also apply to reduced or custom benchmark runs under directories such as:

- `results/smoke/`
- `results/cluster/.../`
- `/mnt/e/data/pdp_retrieve_engine/results/...`
- any alternate `BENCH_RESULTS_DIR` you point the scripts at

When a physical-cluster run also goes through the dedicated postprocess flow, the same raw CSV files may sit beside additional derived outputs such as:
...
### 5. Cluster and smoke subdirectories use the same schemas

If you run the benchmark scripts with a custom `BENCH_RESULTS_DIR`, with `BENCH_STORAGE_ROOT=/mnt/e/data/pdp_retrieve_engine`, or if you keep a manual physical-cluster run under a directory such as `results/cluster/2026-06-22-local-plus-199/`, the filenames may move, but the CSV headers and column meanings do not change.
...
The same header also applies when you store one-run summaries under a dedicated cluster-run folder, for example:
...
- cluster-scoped PNG figures generated from the raw cluster benchmark CSVs
- these do not change the CSV contracts explained in this document; they are a visual layer on top of the same raw tables
